...
def test_complete_receipt_binds_every_presented_byte(tmp_path):
...
def test_partial_receipt_preserves_unpresented_ranges(tmp_path):
    root = tmp_path / "environment"
    root.mkdir()
    ...
    receipt = create_presentation_receipt(
        ...
        "large.bin",
        method="bounded-byte-review-v1",
        presented_ranges=[(2, 5), (7, 10)],
    )

    assert receipt["presented_byte_count"] == 6
    assert receipt["complete_byte_presentation"] is False
    assert receipt["unpresented_ranges"] == [
        {"start": 0, "end": 2},
        {"start": 5, "end": 7},
    ]
    assert verify_presentation_receipt(root, receipt) == receipt
...
def test_empty_file_can_have_complete_byte_presentation(tmp_path):
    root = tmp_path / "environment"
    root.mkdir()
...
def test_invalid_coverage_ranges_are_rejected(tmp_path, ranges):
    ...
    with pytest.raises(PresentationReceiptError):
        create_presentation_receipt(
            root,
            "source.bin",
            method="bounded-byte-review-v1",
...
def test_method_must_be_a_nonempty_string(tmp_path, method):
    ...
    with pytest.raises(PresentationReceiptError, match="method"):
        create_presentation_receipt(
            root,
            "source.txt",
            method=method,
...
def test_tampered_receipt_is_rejected(tmp_path):
    ...
    tampered["presented_ranges"][0]["sha256"] = "0" * 64

    with pytest.raises(PresentationReceiptError, match="receipt hash mismatch"):
        verify_presentation_receipt(root, tampered)
...
def test_receipt_becomes_stale_when_source_changes(tmp_path):
    ...
    with pytest.raises(PresentationReceiptError, match="source identity mismatch"):
        verify_presentation_receipt(root, receipt)


def test_complete_current_receipt_marks_file_presented(tmp_path):
    root = tmp_path / "environment"
    root.mkdir()
    (root / "source.txt").write_bytes(b"source")
    receipt = create_presentation_receipt(
    ...
    coverage = observe_environment(
        root,
        presentation_receipts=[receipt],
    )

    assert coverage["files"][0]["presentation_status"] == "PRESENTED"
    assert coverage["files"][0]["presentation_receipt_hash"] == receipt["receipt_hash"]
...
def test_partial_receipt_does_not_mark_file_presented(tmp_path):
    root = tmp_path / "environment"
    root.mkdir()
    (root / "source.txt").write_bytes(b"source")
    receipt = create_presentation_receipt(
    ...
    assert coverage["files"][0]["presentation_status"] == "PARTIAL"
    assert coverage["presented_file_count"] == 0
    assert coverage["unpresented_file_count"] == 1
    assert coverage["byte_presentation_complete"] is False
...
def test_stale_receipt_is_exposed_without_aborting_observation(tmp_path):
    ...
    assert coverage["files"][0]["presentation_status"] == "STALE"
    assert coverage["presented_file_count"] == 0
    assert coverage["byte_presentation_complete"] is False
    assert coverage["stale_presentation_receipts"] == [receipt["receipt_hash"]]
